...
def fetch_stock_revenue(stock_id, start_date, token):
    try:
        r = requests.get(FINMIND_URL, params={"dataset":"TaiwanStockMonthRevenue","data_id":stock_id,"start_date":start_date,"token":token}, timeout=10)
        data = r.json()
...
def fetch_gross_margin(stock_id, token):
    try:
        r = requests.get(FINMIND_URL,params={"dataset":"TaiwanStockFinancialStatements","data_id":stock_id,"start_date":"2023-01-01","token":token},timeout=10)
...
def run_growth_scanner(df,min_yoy,min_slope,months_back,token):
    ...
    for sid,grp in df.groupby("stock_id"):
        ...
        yoy=grp["yoy_pct"].fillna(0).values; slope=float(np.polyfit(range(len(yoy)),yoy,1)[0])
...
def run_fake_detector(candidates,token,threshold):
    ...
    bar.empty(); result=candidates.head(total).copy(); result["毛利率狀態"]=risks
    return result

@st.cache_data(ttl=1800)
def fetch_institutional(stock_id,start_date,token):
    try:
        r=requests.get(FINMIND_URL,params={"dataset":"TaiwanStockInstitutionalInvestorsBuySell","data_id":stock_id,"start_date":start_date,"token":token},timeout=10)
        data=r.json()
        if data.get("status")!=200: return pd.DataFrame()
        ...
        # ★ 核心修正：用英文 name 對應中文類別
        raw["類別"]=raw["name"].map(INST_NAME_MAP)
        raw=raw.dropna(subset=["類別"])
        # 以日期+類別加總（Dealer_self和Dealer_Hedging都合入「自營」）
...
def analyze_institutional(stock_id,df,days):
    ...
    return {"stock_id":stock_id,"外資淨買(張)":round(recent["外資買賣超(張)"].sum(),0),
...
def cross_compare(m1_df,m2_df):
    ...
    m2_sub=m2_df[m2_df["stock_id"].isin(common)][["stock_id","三大合計(張)","連續買超天","買超比例","信號強度"]].rename(columns={"stock_id":"股票代號"})
...
with st.sidebar:
    ...
    st.markdown("<div style='background:#0d2015;border-left:3px solid #56d364;padding:8px 10px;border-radius:4px;font-size:11px;color:#8ba3b8;'>✅ v2.3：法人欄位對應已修正<br>外資／投信／自營正確計算</div>",unsafe_allow_html=True)
    ...
    st.divider()
    st.markdown(f"<div style='font-size:11px;color:#8ba3b8;'>💡 Token 消耗預估<br>M1：約{len(WATCHLIST)}次 M2：約10~20次<br>合計：約<strong style='color:#56d364;'>{len(WATCHLIST)+20}次</strong></div>",unsafe_allow_html=True)
    st.divider()
...
st.title("🐋 大戶思維投資導航系統")
st.caption("Module 1 成長動能 × Module 2-1 三大法人 ｜ v2.3")

tab_cross,tab_m1,tab_m2,tab_guide,tab_road=st.tabs(["🎯 交叉比對","📈 Module 1","📊 Module 2","📖 使用說明","🗺 開發路線圖"])

with tab_guide:
    st.markdown("""
### 📱 操作方式
1. 左側輸入 **FinMind Token**（必填）  2. 調整參數  3. 按「🚀 開始分析」  4. 看「🎯 交叉比對」

### 📊 v2.3 三大法人欄位說明
| API 欄位 | 對應類別 | 說明 |
|---------|---------|------|
| Foreign_Investor | 外資 | 外資法人 |
| Investment_Trust | 投信 | 投信法人 |
| Dealer_self + Dealer_Hedging | 自營 | 自營商合計 |
...
if not token_to_use:
    with tab_cross:
        st.markdown('<div class="alert-blue"><strong>🔑 請先輸入 FinMind Token</strong></div>',unsafe_allow_html=True)
elif not run_btn:
    with tab_cross:
        st.markdown('<div style="text-align:center;padding:60px 0;color:#506880;"><div style="font-size:56px">🐋</div><p style="font-size:18px;margin-top:16px;">Token 已就緒，按左側「開始分析」</p><p style="font-size:12px;">v2.3：法人欄位已修正，約 2～3 分鐘</p></div>',unsafe_allow_html=True)
else:
    ...
    with tab_m1:
        ...
        if not m1_result.empty: m1_result=run_fake_detector(m1_result,token_to_use,margin_threshold)
        c1,c2,c3,c4=st.columns(4)
        m1_passed=len(m1_result)
        m1_flagged=len(m1_result[m1_result["毛利率狀態"].str.contains("⚠️",na=False)]) if "毛利率狀態" in m1_result.columns else 0
    ...
    with tab_m2:
        ...
        st.markdown(f'<div class="alert-green">✅ v2.3：只針對 M1 篩出的 <strong>{m1_count} 支候選</strong>做法人追蹤</div>',unsafe_allow_html=True)
        ...
        if 'm1_result' not in dir() or m1_result.empty:
            ...
            if m2_result.empty:
                st.markdown('<div class="alert-yellow">M1 候選中目前無股票符合法人買超條件。可試著降低門檻，或明天再試。</div>',unsafe_allow_html=True)
            else:
                ...
                m2_cols=[c for c in ["stock_id","公司名稱","三大合計(張)","外資淨買(張)","投信淨買(張)","自營淨買(張)","連續買超天","買超比例","信號強度"] if c in m2_result.columns]
                def hl2(val):
                ...
                styled2=(m2_result[m2_cols].style.map(hl2,subset=["信號強度"])
                         .format({"買超比例":"{:.1f}%","三大合計(張)":"{:.0f}","外資淨買(張)":"{:.0f}","投信淨買(張)":"{:.0f}","自營淨買(張)":"{:.0f}"})
